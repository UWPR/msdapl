...
//    }
    ...
    private int uploadSearch(int experimentId, S search) throws UploadException {
        ...
        if (sequenceDatabaseId == 0) {
            sequenceDatabaseId = parseAndUploadFasta();
        }
    ...
    private int parseAndUploadFasta() throws UploadException {
        ...
        try {
            ...
            log.info("No species ID found for " + noSpeciesFoundCount + " proteins");
        }
        catch(Exception e)
        {
        ...
        {
            try {if(conn != null) conn.close();} catch (Exception e) {System.out.println("Error closing connection. " + e);}
        }
        ...
        try
        {
            ...
            int yrcNrseqId = getSearchDatabaseId(db);
            log.info("Uploaded fasta database. YRC_NRSEQ id: " + yrcNrseqId);
            return yrcNrseqId;
        }
        catch(Exception e)
        {
